...
function soldesDuColis(c) {
  ...
  const out = [];
  if (c.article_non_encaisse && montantArticleColis(c)) out.push(Object.assign({ cle: 'article', quoi: 'Article', pourquoi: 'soldé chez le vendeur : le livreur ne l’encaisse pas', montant: montantArticleColis(c) }, qui('article_non_encaisse')));
  if (c.livraison_payee && montantLivraisonColis(c)) out.push(Object.assign({ cle: 'livraison', quoi: 'Livraison', pourquoi: 'payée d’avance chez le vendeur : retenue sur son relevé, pas encaissée par le livreur', montant: montantLivraisonColis(c) }, qui('livraison_payee')));
...
/* Chez le vendeur (Celtis, 25/09 : « chez la vendeuse ce n'est pas nécessaire, il suffit qu'elle
...
function soldesResumeHTML(colis) {
  ...
  const parts = [];
  if (r.article) parts.push('articles soldés chez le vendeur ' + m(r.article));
