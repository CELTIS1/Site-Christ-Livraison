...
(function () {
  ...
  function ldQualifierHTML(liste) {
    ...
    return `<div class="ld-qual-liste">${liste.slice(0, 60).map((c) => {
      ...
      return `<div class="ld-qual" data-ld-colis="${esc(c.id)}">
        ...
        <div class="ld-qual-motif">${esc(motifTexte(c.motif_non_livraison))} · ${c.vendeuse_prevenue ? '✅ vendeur prévenu' : '⚠️ vendeur pas prévenu'} · ${Number(c.tentatives_livraison || 0)} passage${Number(c.tentatives_livraison || 0) > 1 ? 's' : ''}</div>
  ...
  function ldFicheHTML(l) {
    ...
    const sourceLib = { cliente: 'cliente', vendeur: 'vendeuse', equipe: 'équipe', autre: 'autre' };
    return `
      ...
      <div class="cd-bloc"><h3 class="panel-subtitle">Réclamations</h3>
        ...
        <details class="ld-recl-ajout"><summary>+ Enregistrer une réclamation</summary>
          <div class="ld-recl-form">
            <select id="ld-recl-source"><option value="cliente">D’une cliente</option><option value="vendeuse">D’un vendeur</option><option value="equipe">De l’équipe</option><option value="autre">Autre</option></select>
