...
// 25/09/2026 (lot 13) : les deux côtés deviennent six vues (Tout · Non livrés · Retours · Reportés · Signalements · Demandes), chacune avec son compte.
verifier('six vues, comme « Personnes » : Tout, Non livrés, Retours, Reportés, Signalements, Demandes, chacune avec son compte', /data-rt-vue="tout"/.test(page) && /data-rt-vue="retours"/.test(page) && /data-rt-vue="non_livres"/.test(page) && /data-rt-vue="reportes"/.test(page) && /data-rt-vue="signalements"/.test(page) && /data-rt-vue="demandes"/.test(page) && /id="rt-n-retours"/.test(page) && /id="rt-n-non-livres"/.test(page) && /class="eq-bascule rt-bascule rt-genres"/.test(page));
verifier('l\'écran ne montre que la vue ouverte, et s\'en souvient', /rtVue === 'tout' \? tous : tous\.filter\(l => l\.genre === rtVue\)/.test(ecran) && /clt_equipe_retours_vue/.test(ecran));
...
verifier('le chiffre de l\'onglet compte chaque colis UNE fois (un litige en retard ne vaut pas deux) — compté par la règle', /const urgent = n\.urgent;/.test(ecran) && /if \(l\.urgence < 1\) n\.urgent \+= 1;/.test(lire('app/a-traiter.js')));
