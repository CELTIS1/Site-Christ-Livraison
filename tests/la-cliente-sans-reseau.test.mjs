...
verifier('le détail d\'une boutique ne fait défiler que s\'il n\'est pas sous les yeux', /if \(r\.top < 0 \|\| r\.top > window\.innerHeight \* 0\.6\) detail\.scrollIntoView/.test(fo));

console.log('\n3. La nuit et les couleurs');
verifier('les variables --tile-color / --tile-bg sont enfin lues, et la nuit garde un fond sombre', /\.stat-tile\{ background:var\(--tile-bg, var\(--white\)\); \}/.test(css) && /html\[data-theme="dark"\] \.stat-tile\{ background:#1f2a3a/.test(css));
verifier('le numéro, la zone « coller », les pastilles des boutiques, le montant absent ont leur mode nuit', /html\[data-theme="dark"\] \.colis-tel\{ color:#9EC0E6; \}/.test(fo) && /html\[data-theme="dark"\] \.coller-zone\{/.test(fo) && /html\[data-theme="dark"\] \.mb-chiffre--livre\{/.test(css) && /html\[data-theme="dark"\] \.montant-absent\{/.test(css));
