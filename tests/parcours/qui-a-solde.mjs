...
verifier('trois lignes « 🔖 » sous les colis : article soldé (Awa, à la saisie), livraison payée d\'avance (Roxy, bureau), manque (auteur non enregistré)', lignes.length === 3 && lignes.some((t) => /Article 15\s?000 FCFA soldé chez le fournisseur.*coché à la saisie par Awa Boutique \(cliente\)/.test(t)) && lignes.some((t) => /payée d’avance.*coché par Roxy \(bureau\)/.test(t)) && lignes.some((t) => /manque.*auteur non enregistré/.test(t)), JSON.stringify(lignes));
const resume = (await page.locator('.soldes-resume').innerText()).replace(/\s+/g, ' ');
verifier('sous le TOTAL : « Pas encaissé par le livreur : 3 colis », les trois montants, les numéros', /Pas encaissé par le livreur : 3 colis/.test(resume) && /articles soldés chez le fournisseur 15\s?000 FCFA/.test(resume) && /CLT-260916-00403/.test(resume), resume);
