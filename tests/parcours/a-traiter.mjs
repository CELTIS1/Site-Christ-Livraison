...
// 25/09/2026 (lot P-2) : septième vue, « Litiges Express » (vide ici).
verifier('sept vues avec leur compte : Tout · Non livrés · Retours (0) · Reportés 1 · Signalements 1 · Demandes 1 · Litiges Express (0)', (await vues.count()) === 7 && /Litiges Express/.test(await texte(vues.nth(6))) && (await texte(vues.nth(0))) === 'Tout ' + nTout && (await texte(vues.nth(1))) === '⚠️ Non livrés ' + nNl && (await texte(vues.nth(2))) === '↩️ Retours' && /Reportés 1/.test(await texte(vues.nth(3))) && /Signalements 1/.test(await texte(vues.nth(4))) && /Demandes 1/.test(await texte(vues.nth(5))), await texte(page.locator('#section-retours .rt-genres')));
