...
titre('4. L\'amélioration constante : le rapport d\'usage, lancé maintenant, lu dans la carte');
verifier('l\'administrateur a deux boutons « maintenant » (semaine, mois), ≥ 44 px', (await page.locator('#rap-carte [data-rap-generer]').count()) === 2 && await page.locator('#rap-carte [data-rap-generer="usage_semaine"]').evaluate((b) => b.getBoundingClientRect().height >= 44));
await page.evaluate(() => document.querySelector('#rap-carte [data-rap-generer="usage_semaine"]').click());
await dodo(800);
verifier('« Rapport en rédaction » : la base a reçu rapport_usage_creer', /Rapport en rédaction/.test(await page.evaluate(() => document.body.innerText)) && (monde.TABLES.rapports_usage || []).length === 1);
await page.evaluate(() => window.CLTRapportsRecusEcran.charger()); await dodo(1200);
const usage = page.locator('#rap-carte > .rap-ligne[data-rap="ru-3"]');
