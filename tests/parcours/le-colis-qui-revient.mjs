...
verifier('la carte des retours est sur l\'accueil', (await carteRt.count()) === 1 && await carteRt.isVisible());
...
verifier('le résumé compte : 1 en retard, chez les livreurs, 1 à confirmer, et le non livré du monde', /1 en retard/.test(resume) && /à confirmer/.test(resume) && /non livré/.test(resume), resume);
...
await bandeau.click();
await dodo(800);
const carteC = (id) => page.locator(`#colis-list .colis-item[data-id="${id}"]`).first();
verifier('la liste ne montre plus que ses retours, tous jours confondus', (await carteC(REVIENT.id).count()) === 1 && (await carteC(TRAINE.id).count()) === 1 && (await page.locator('#colis-list .colis-item').count()) === 2, String(await page.locator('#colis-list .colis-item').count()));
