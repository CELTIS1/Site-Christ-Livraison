...
vm.runInContext(blocDe(config, 'jourAbidjan') + '\n' + blocDe(config, 'jourEvenementColis') + '\n'
  ...
  + blocDe(config, 'colisDeLaJourneeDeTravail'), ctx);
...
titre('Les deux écrans du livreur passent par cette définition');
{
  ...
  verifier('la liste « Ma journée » appelle colisDeLaJourneeDeTravail', /colisDeLaJourneeDeTravail\(mine, todayLocalISODate\(\)\)/.test(rendu));
  ...
  verifier('les tuiles comptent la journée de travail, pas le jour de réception', /colisDeLaJourneeDeTravail\(mine, jour\)/.test(tuiles) && !/dayKey\(c\.created_at\) === jour\)/.test(tuiles.replace(/\/\/[^\n]*/g, '')));
