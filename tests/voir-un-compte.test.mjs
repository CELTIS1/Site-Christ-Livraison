...
verifier('les lectures personnelles passent par la règle, et nulle part ailleurs', /const sort = R\.sortDeLOperation\('rpc', nom, compte\);/.test(verrou) && /const d = R\.lectureDeLaBase\(nom, args, compte\); return vraiRpc\(d\.nom, d\.args\);/.test(verrou));
