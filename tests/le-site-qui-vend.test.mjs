...
verifier('la preuve en chiffres (revue v285) : trois tuiles — colis et fournisseurs arrondis par site_chiffres (data-chiffre, les mêmes qu\'« À propos »), le taux par site_chiffres_mois ; aucun montant ; une seule chargerChiffres ; tirets sans réseau', /<strong data-chiffre="colis_livres">1 000\+<\/strong><span>colis livrés à Abidjan/.test(index) && /<strong data-chiffre="commercants">60\+<\/strong><span>fournisseurs nous font confiance/.test(index) && /id="chiffre-taux">—</.test(index) && !/chiffre-reverse|chiffre-vendeuses|chiffre-livres-mois|reverse_mois/.test(index) && /rest\/v1\/rpc\/site_chiffres_mois/.test(index) && /chargerChiffresDuMois\(\);/.test(index) && (index.match(/function chargerChiffres\(\)/g) || []).length === 1 && /les tirets restent/.test(index));
...
verifier('le même nombre partout (v285) : colis et fournisseurs du bandeau et d\'« À propos » lus au même endroit, même libellé ; « fournisseurs » et non « vendeuses »', (index.match(/data-chiffre="colis_livres">1 000\+</g) || []).length === 2 && (index.match(/data-chiffre="commercants">60\+</g) || []).length === 2 && (index.match(/fournisseurs nous font confiance/g) || []).length === 2 && contenuSite.about.stats[0].label === 'fournisseurs nous font confiance' && contenuSite.about.stats[1].number === '{colis_livres|1 000+}' && /Fournisseur de la semaine/.test(index) && !/Vendeuse de la semaine</.test(index));
...
verifier('le menu de l\'accueil mène à Fournisseurs (page vendeuses.html) et à Livreurs', /href="vendeuses\.html" onclick="closeMenu\(\)">Fournisseurs/.test(index) && /href="livreurs\.html" onclick="closeMenu\(\)">Devenir livreur/.test(index));
