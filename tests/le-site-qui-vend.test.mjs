...
verifier('le produit dit sans être montré (26/09) : trois atouts, « Demander une démonstration » ; aucune vidéo de l\'application', (index.match(/class="produit-atout /g) || []).length + (index.match(/class="produit-atout"/g) || []).length === 3 && /Demander une démonstration/.test(index) && !/aide\/videos\/cliente-/.test(index) && /media-src 'self'/.test(index));
...
// 26/09/2026 : plus de vidéos de l'application sur le site (discrétion) ; elle se découvre à l'entretien.
verifier('plus de vidéo de l\'application sur la page Livreurs ; « Ton outil de travail » la dit sans la montrer', !/<video/.test(livreurs) && /Ton outil de travail/.test(livreurs));
...
verifier('vendeuses.html : ce que vous gagnez, trois temps, AUCUNE vidéo de l\'application (26/09 : « Voir l\'application en 10 minutes » sur WhatsApp), le prix, « Nos vendeuses » lu par site_vendeuses (anon), filtres par secteur, « Commander chez elle » sur WhatsApp', /id="gagnez"/.test(vendeuses) && !/<video/.test(vendeuses) && /Voir l'application en 10 minutes/.test(vendeuses) && /rpc\/site_vendeuses/.test(vendeuses) && /data-filtre/.test(vendeuses) && /Commander chez elle/.test(vendeuses) && /je vous ai trouvée sur le site de CLT/.test(vendeuses));
...
verifier('express.html : plus de vidéos ni de formule de prix (26/09), les objections, le code de livraison expliqué, media-src', !/<video/.test(express) && !/150 FCFA\/km|150 F par kilomètre/.test(express) && /objections-express/.test(express) && /code à 4 chiffres/.test(express) && /media-src 'self'/.test(express));
