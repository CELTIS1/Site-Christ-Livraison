...
verifier('les gestes du bureau (seconde moitié) : une ligne par geste possible, un choix de coursier pour attribuer / confier, confirmation, motif d\'annulation obligatoire, trace bureau_geste + journal, repli sans les colonnes',
  /function expressGestesHTML\(course\)/.test(js) && /D\.gestesDuBureau\(course\)/.test(js) && /data-express-geste="\$\{g\.cle\}"/.test(js) && /express-geste__coursier/.test(js) && /cltPrompt\(\{ title: 'Annuler la course'/.test(js) && /bureau_geste: geste, bureau_geste_par: moi, bureau_geste_at: maintenant/.test(js) && /express_bureau_' \+ geste/.test(js) && /schema cache/.test(js));
verifier('la fenêtre du dossier est réutilisée quand il se rafraîchit (pas de retrait + recréation : l\'historique reculait de deux crans)', /let ov = document\.getElementById\('express-dossier'\);\s*if \(!ov\)/.test(js));
verifier('style : chronologie en frise, personnes en grille, gestes 44 px, nuit', /\.express-chrono li::before/.test(css) && /\.express-dossier__personnes\{ display:grid/.test(css) && /\.express-geste \.btn\{ min-height:44px/.test(css) && /html\[data-theme="dark"\] \.express-dossier__personnes > div/.test(css));
