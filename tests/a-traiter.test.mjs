...
verifier('11 lignes : 2 non livrés, 3 retours, 2 reportés, 2 signalements, 2 demandes', L.length === 11 && JSON.stringify(A.compterParGenre(L)) === '{"tout":11,"non_livres":2,"retours":3,"reportes":2,"signalements":2,"demandes":2,"litiges":0,"sans_coursier":0,"urgent":2}', JSON.stringify(A.compterParGenre(L)));
...
// 25/09/2026 (chantier P, lot P-2) : septième vue, les litiges Express ; (lot P-4) huitième, les courses sans coursier.
verifier('une aide par vue, huit vues', Object.keys(A.AIDE).join(',') === 'tout,non_livres,retours,reportes,signalements,demandes,litiges,sans_coursier' && A.GENRES.length === 8);
const T0 = Date.parse(AUJ + 'T10:00:00Z');
const SC = A.lignesATraiter({ coursesExpress: [
  { id: 'k1', status: 'en_attente', coursier_id: null, created_at: new Date(T0 - 12 * 60000).toISOString(), description_colis: 'Un carton', adresse_recuperation: 'A', adresse_livraison: 'B', dispatch_vague: 3, dispatch_rayon_km: 12, bureau_alerte_at: new Date(T0 - 60000).toISOString(), client_telephone: '2250700000001' },
  { id: 'k2', status: 'en_attente', coursier_id: null, created_at: new Date(T0 - 2 * 60000).toISOString() },
  { id: 'k3', status: 'acceptee', coursier_id: 'x', created_at: new Date(T0 - 30 * 60000).toISOString() },
  { id: 'k4', status: 'en_attente', coursier_id: null, created_at: new Date(T0 - 11 * 60000).toISOString() },
] }, AUJ, { maintenant: T0 });
verifier('sans coursier : la course alertée par la base et celle de 11 min ; pas celle de 2 min ni l\'acceptée ; elles brûlent, « depuis 12 min »', SC.length === 2 && SC.every(l => l.genre === 'sans_coursier' && l.urgence === 0) && SC[0].id === 'k1' && SC[0].depuis === 'depuis 12 min' && SC[1].id === 'k4', JSON.stringify(SC.map(l => [l.id, l.depuis])));
verifier('« Que faire ? » sans coursier : ouvrir le dossier (attribuer / annuler), appeler le client quand on a son numéro', A.choixQueFaire(SC[0]).map(c => c.cle).join(',') === 'dossier,appeler' && A.choixQueFaire(SC[1]).map(c => c.cle).join(',') === 'dossier');
verifier('courseSansCoursier : alerte de la base ou 10 min, jamais une course prise', A.courseSansCoursier({ status: 'en_attente', created_at: new Date(T0 - 60000).toISOString(), bureau_alerte_at: 'x' }, T0) && !A.courseSansCoursier({ status: 'en_attente', created_at: new Date(T0 - 9 * 60000).toISOString() }, T0) && !A.courseSansCoursier({ status: 'acceptee', coursier_id: 'x', created_at: new Date(T0 - 99 * 60000).toISOString() }, T0));
...
verifier('la liste vient de lignesATraiter avec retourNiveau / retourEnRetard / retourDepart', /A\.lignesATraiter\(\{ colis: rtColis, reportes: rtReportes, reclamations: [^}]+, demandes: rtDemandes, litiges: [^}]+, coursesExpress: [^}]+ \}, todayLocalISODate\(\), \{ retourNiveau, retourEnRetard, retourDepart \}\)/.test(ecran));
