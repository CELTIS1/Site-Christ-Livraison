...
verifier('11 lignes : 2 non livrés, 3 retours, 2 reportés, 2 signalements, 2 demandes', L.length === 11 && JSON.stringify(A.compterParGenre(L)) === '{"tout":11,"non_livres":2,"retours":3,"reportes":2,"signalements":2,"demandes":2,"urgent":2}', JSON.stringify(A.compterParGenre(L)));
...
verifier('une aide par vue, six vues', Object.keys(A.AIDE).join(',') === 'tout,non_livres,retours,reportes,signalements,demandes' && A.GENRES.length === 6);
...
verifier('la liste vient de lignesATraiter avec retourNiveau / retourEnRetard / retourDepart', /A\.lignesATraiter\(\{ colis: rtColis, reportes: rtReportes, reclamations: [^}]+, demandes: rtDemandes \}, todayLocalISODate\(\), \{ retourNiveau, retourEnRetard, retourDepart \}\)/.test(ecran));
